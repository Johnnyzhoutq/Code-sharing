...
public class ValidateUtil {
	...
	/** 身份证  */
  	...
  	/**********************数据通用校验工具类***********************************/
  	/**
	 * 校验map中的指定keys的对象是否为空,如果为String需判断是否为空串 
	 * @param keys
	 * @return 为空的对象key 如果都不为空则返回size为0 的Set对象
	 */
	public static Set<String> validateMapNullValueByKeys(Map validateMap,Set keys){
		...
		String key = iter.next();
			Object valueObj = validateMap.get(key);
			if(valueObj == null){
				returnSet.add(key);
			}else if(valueObj instanceof String ){
				if(StringUtil.isEmpty(valueObj.toString())){
					returnSet.add(key);
				}
			}
		}
	 ...
	 * @return 为空的对象key 如果都不为空则返回size为0 的Set对象
	 */
	public static Set<String> validateMapNullValueByKeys(Map validateMap,String[] keys){
		Set returnSet = new HashSet();
		if(keys == null){
			return returnSet;
		}
